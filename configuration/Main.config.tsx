    ...
    introduce: [
        {
            ...
            description: ' 나름대로 오랜시간동안 다양한 개발 분야를 공부해왔습니다. \
            하지만그 어떤 분야에서도 진지하게 임하지 못하고 떠돌기만 하던 중 네이버 부스트 캠프를 통해 내가 개발로써 만들고자 \
            하는 것은 내가만든 무언가가 직접적으로 사용자와 소통하게 하는것이라는 것을 알게되었고 이를 구현하기 위한 가장 \
            중요한 기술은 프론트엔드임을 깨달아 도전하여 지금에 이르렀습니다.',
        },
        {
            title: '프론트엔드의 핵심은 사용자의 즐거움',
            import: '사용자가 즐겁게 사용할 수 있는 개발을 중시합니다.',
            description: ' 즐거운 사용은 단순하지 않다고 생각합니다. \
            퍼포먼스, 디자인, 애니메이션과 반응성, 상호작용등 모든 요소가 적절히 조합되고 균형을 맞출 때 비로소 진정한 프론트엔드 \
            개발자의역량을 드러낼 수 있다고 생각합니다. 따라서 단순한 코드를 넘어선 유저와의 상호작용과 성능을 고려한 컴포넌트 \
            디자인을 하기 위해 노력하고 공부하고 있습니다.'
        },
        {
            title: '재사용, 그것은 나의 목표',
            import: '항상 코드를 짜기 전, 혹은 짜면서 항상 \
            컴포넌트를 지속적으로 재사용할 수 있게끔 작성하도록 노력합니다.',
            description: ' 코딩을 하면서 몇번이나 했던 작업을반복하게 되는 것은 엄청난 비효율을 초래합니다. 이를 방지하는 노력을 부단히 하기 \
            위해 개발 전 컴포넌트의 흐름을그리고 라이브러리화 할 수 있는 요소들을 파악하려고 노력합니다. \
            최종적으로는 configuration만으로 모든 패턴을 변경할 수 있는 컴포넌트를 만드는 것을 목표로 합니다.'
        },
        {
            title: '제일 중요한 것은 워크플로우와 아카이빙',
            import: '팀과 작업의 방향을 맞추고 어떻게 자료를 공유할지 규정하는 \
            것을 굉장히 중요하게 생각합니다.',
            description: ' 팀과 작업의 방향을 맞추고 어떻게 자료를 공유할지 규정하는 것을 굉장히 중요하게 생각합니다. \
            부끄럽지만 얼마 전 까지만 하더라도 전혀 중요하게 생각하지 못했지만 팀 작업의 총괄 관리를 해보면서 단순히 툴(git이나 \
            trello)을이용하는 것 뿐만 아닌 서로의 커뮤니케이션을 맞추고 워크플로우를 조직화하는 것의 중요성을 알게 되었고 이 후 \
            모든 작업을 하기 전 서로의 커뮤니케이션 툴부터 작업 관리 방식을 모두 문서화하는 작업을 수행합니다. 개발은 혼자서 할 \
            수 없기에 모두의 커뮤니케이션을 맞추는 것만큼 중요한 것은 없다고 생각합니다.'
        }
    ]
}
